...
def hf_llm_inference(model_name, llm_model, prompt, device='cuda',tokenizer=None, max_length=80, output_scores=False, processors=None, temperature=1.0):
    """
    hugging-face llm-inference function : 
        args 
            - model_name    : language model name
            - llm_model     : loaded hugging-face model object 
            - prompt        : input prompt 
            - max_length    : max_length 
            - output_scores : scoring function 
            - processors    : processors ? 
            - temperature   : llm decoding temperature (smooting factor)
    modified by khm for handling llamma, phi
    """
...
def get_mc_dataset(dataset, model, model_name, tokenizer=None):
    num_data = len(dataset)
    for i in tqdm(range(num_data)):
        test_data = dataset[i]
        prompt = test_data['mc_gen_prompt']
        ...
        dataset[i]['mc_gen_raw'] = gen_raw
    return dataset

def get_mc_dataset_gpt(dataset, gpt_model="gpt-4-1106-preview"):
    num_data = len(dataset)
    for i in tqdm(range(num_data)):
        test_data = dataset[i]
        prompt = test_data['mc_gen_prompt']
